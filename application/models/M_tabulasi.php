...
class M_tabulasi extends CI_Model{
    ...
    public function select_detail(){
        if($this->session->userdata('id_role') == 3){
            ...
            $sql = $sql."	(SELECT SUM(sssah) FROM tbl_desa_penyelenggara AS a WHERE a.thn_pemilihan = '". $this->session->userdata('thn_data')."' AND a.kdkec = c.id_kec) AS DSSH, ";
			$sql = $sql."	(SELECT SUM(s_hasil) FROM tbl_calon AS b WHERE b.thn_pemilihan = '". $this->session->userdata('thn_data')."' AND b.kdkec = c.id_kec ) AS SUARAM ";
